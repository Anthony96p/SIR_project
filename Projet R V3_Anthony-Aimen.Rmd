...
Taille <- 300 #Nombre de jours choisi
#S->I->R

#Valeurs spéciales vaccination
Tv <- 0.04 # Taux de vaccination pour un temps donné "fctTv"
FctTv <- 7 #Temps en jour corespondant au taux de vaccination #ici pour 1 semaine

#Valeurs spéciales mortalité
Mu <- 0.02  #proba de mortalité pour un temps donné "fctTm"
FctTm <-30  #Temps en jour corespondant a la proba de mortalité #ici pour 1 mois
...
eulervacci<-function(Sbis,Ibis,Rbis,a,b,deltaT,taille,tv,fctT) # fonction calculant à  chaque tour l’évolution des variables S, I et R
{
  J<-0  #Non modulable       #Jour 0
  ...
  repeat{
    ...
    Sbis <- Suc + (-a * Suc * Infect)*deltaT - (pV/(1/deltaT)) #taux retiré de S à chaque jour
    Rbis <- Reco + (b * Infect)*deltaT + (pV/(1/deltaT)) #taux affecté à R à chaque jour
    Ibis <- (Suc+Infect+Reco) - Sbis - Rbis 
    SIR <- data.frame(Sbis,Ibis,Rbis,J)
    colnames(SIR) <- c("S","I","R","J");
    resul<-rbind(resul,SIR) #concaténation des nouvelles lignes avec la première ligne du tableau
...
resultsvacci<-eulervacci(S ,I ,R,a ,b,DeltaT,Taille,Tv,FctT) #application sur les valeurs de départ
resultsvacci
...
eulermorta<-function(Sbis,Ibis,Rbis,a,b,deltaT,taille,mu,fctTm) # fonction calculant à chaque tour l’évolution des variables S, I et R
{
  J<-0    #Non modulable       #Jour 0
...
resultsmorta<-eulermorta(S ,I ,R,a ,b,DeltaT,Taille,Mu,FctTm) 
resultsmorta
